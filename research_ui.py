...
def _judgement_cards(details):
    reports = [r for _, r, _, _ in details.values() if r]
    financials = [r['financial'] for r in reports if r.get('financial')]
    flows = [r['flow'] for r in reports if r.get('flow')]
    signals = [
        ('종합 투자판단', '평가 보류', '검증된 종합 점수 기준이 필요합니다.', 'gold'),
        ('매크로', '시장 API 대기', '금리·환율 자료가 아직 연결되지 않았습니다.', 'slate'),
        ('성장산업', '조사 필요', '산업 비교 자료를 확인해야 합니다.', 'green'),
        ('수출·수주', '조사 필요', '수출·수주 근거를 확인해야 합니다.', 'green'),
        ('실적 성장', f"{len(financials)}개 기업 확인", '같은 기간의 공시 실적을 아래에서 비교합니다.', 'green'),
        ('수급 강도', f"{len(flows)}개 기업 확인", '조사 자료에 수급 근거가 있는 기업 수입니다.', 'blue'),
    ]
...
def _market_board(details):
    st.caption('시장  ›  산업  ›  기업  ›  투자판단 · 연결되지 않은 항목은 임의 수치를 표시하지 않습니다.')
    _judgement_cards(details)
    overview(details, st.session_state.get('account_snapshot'))
    left, sector, watch = st.columns([2.2, 1, 1], gap='small')
    chart_keys = [key for key, (_, report, _, frame) in details.items()
                  if report and frame is not None and not frame.empty]
    with left, st.container(border=True):
        st.subheader('관심종목 가격 추이')
        if chart_keys:
            selected = st.selectbox('차트 종목', chart_keys,
                                    format_func=lambda key: details[key][0]['name'],
                                    key='market_chart_stock')
            item, report, _, frame = details[selected]
            st.caption('수정종가 · ' + str(frame['date'].iloc[0]) + ' ~ '
                       + str(frame['date'].iloc[-1]) + ' · 실시간 시세가 아닙니다.')
            st.line_chart(frame.set_index('date')['close'], color='#177a53',
                          use_container_width=True, height=280)
            source = (report.get('prices') or {}).get('source')
            if source:
                st.link_button('가격 자료 원문', source)
        else:
            st.info('가격 자료가 수집된 종목을 추가하면 차트가 표시됩니다.')
    with sector, st.container(border=True):
        st.subheader('섹터별 등락률')
        st.caption('시장 API 미연결')
        st.info('섹터 시세가 연결되면 등락률을 표시합니다.')
    with watch, st.container(border=True):
        st.subheader('관심종목')
        for item, report, _, frame in list(details.values())[:7]:
            close = f"{float(frame['close'].iloc[-1]):,.0f}원" if frame is not None and not frame.empty else '가격 미수집'
            st.markdown('**' + html.escape(item['name']) + '** · ' + close)
            st.caption('조사일 ' + report.get('as_of', '미조사'))
        if len(details) > 7:
...
def render_research(store, state, sample_mode):
    ...
    _market_board(details)
    st.markdown('### 내 종목 살펴보기')
    if sample_mode:
        st.info('둘러보기 중입니다. 개인 목록을 저장하려면 먼저 대시보드 비밀번호를 설정하세요.')
    else:
        with st.expander('＋ 종목 추가', expanded=not state.get('stocks')):
